...
if (!file.exists("parameter_lookup.csv")) {
  pt <- data.frame(
    ...
    name = c("CMA_cost", "GP_cost", "ES_cost", "GP_post_n", "GP_post_p", "ES_post_n", "ES_post_p",
             "penalty", "GP_yield", "ES_yield", "expert_GP_tn", "expert_GP_tp",
    ...
    display_name = c("CMA test cost", "Gene-panel test cost", "Exome sequencing test cost",
                     ...
                     "Exome Sequencing(negative)", "Exome Sequencing(positive)", "FN penalty",
                     "Gene-panel", "Exome Sequencing", "Expert TN rate (GP)", "Expert TP rate (GP)",
                     "Expert TN rate (ES)", "Expert TP rate (ES)", "AI TN rate (GP)", "AI TP rate (GP)",
                     "AI TN rate (ES)", "AI TP rate (ES)"),
    default_value = c(500, 1500, 3000, 500, 1000, 1500, 1500, 2000, 0.2, 0.3, 0.6, 0.65, 0.7, 0.75, 0.6, 0.65, 0.7, 0.75),
    ...
    group = c(rep("Cost", 8), rep("Diagnostic Yield", 2), rep("Expert Performance", 4), rep("AI Performance", 4)),
...
generateParameterTable <-function(input){
  ...
  value = c(input$CMA_cost, input$GP_cost, input$ES_cost, input$GP_post_n, input$GP_post_p, input$ES_post_n, input$ES_post_p,
            input$penalty, input$GP_yield, input$ES_yield, input$expert_GP_tn, input$expert_GP_tp,
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      actionButton("reset", "Reset all parameters")
    ),
    mainPanel(
      h3("Introduction"),
